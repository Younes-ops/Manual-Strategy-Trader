        ...
        if self.state == 'AWAITING_SPREAD' and msg.body['msg'] == 'QUERY_SPREAD':
            ...
            if dt < 25:
                ...
                if bid and ask:
                    ...
                    if len(self.mid_list) > self.window3: 
                        self.avg_win3_list.append(pd.Series(self.mid_list).ewm(span=self.window3).mean().values[-1].round(2))
                    if len(self.avg_win1_list) > 0 and len(self.avg_win2_list) > 0 and len(self.avg_win3_list) > 0 and len(self.orders) == 0:
                        buy_signal = self.avg_win2_list[-1] > self.avg_win1_list[-1]
                        sell_signal = self.avg_win2_list[-1] < self.avg_win1_list[-1]
                        if buy_signal :
